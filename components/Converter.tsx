...
export const Converter: React.FC<ConverterProps> = ({ rates }) => {
  ...
  const getRateInNGN = (code: string): number => {
    ...
    // Use average of buy/sell for calculation estimate
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-6 border border-slate-100 dark:border-slate-800 relative transition-colors duration-300">
      ...
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr,auto,1fr] gap-4 items-start">
             ...
             <div className="space-y-2">
                ...
                <div className="relative">
                    <input
                        ...
                        className="block w-full rounded-xl border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-4 text-2xl font-bold text-slate-900 dark:text-white focus:border-emerald-500 focus:ring-emerald-500 dark:focus:ring-emerald-500/50 transition-colors"
                        placeholder="0.00"
                    />
                </div>
            </div>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
                ...
                <div className="relative block w-full rounded-xl border border-emerald-100 dark:border-emerald-900/30 bg-emerald-50/50 dark:bg-emerald-900/10 p-4 text-right h-[74px] flex items-center justify-end">
                    <div className="text-2xl font-bold text-emerald-900 dark:text-emerald-400 break-all line-clamp-1">
        ...
        <div className="mt-8 text-center">
            <p className="text-sm text-slate-500 dark:text-slate-400 inline-flex items-center bg-slate-50 dark:bg-slate-800/50 px-3 py-1 rounded-full">
                1 {fromCurrency} ≈ {(getRateInNGN(fromCurrency) / getRateInNGN(toCurrency)).toLocaleString(undefined, {maximumFractionDigits: 4})} {toCurrency}
                <span className="mx-2 text-slate-300">•</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-medium">Parallel Market Rate</span>
            </p>
        </div>
      </div>
    </div>
